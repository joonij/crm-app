...
const mapToStandardCoverage = (rawName: string) => {
  ...
  if (rawName.includes("주계약") || rawName.includes("[W]") || rawName.includes("특약W")) return rawName;
        ...
        for (const line of lines) {
          ...
          if (len >= 6) {
            ...
            if (isPremium && isValidPayTerm) {
              ...
              if (name.length > 0) {
                extractedDetails.push({ name: mapToStandardCoverage(name), amount: formatAmountWithComma(amount), renewal_type: renewal });
              }
            }
          }
        }
        ...
        for (const line of lines) {
          ...
          if (len >= 4) {
            ...
            if (isPremium && isValidPayTerm) {
              ...
              if (name.length > 0) extractedDetails.push({ name: mapToStandardCoverage(name), amount: formatAmountWithComma(amount), renewal_type: renewal });
            }
          }
        }
      } 
